module TS
{
  ...
  export module Security
  {
    ...
    /**
    * @description Slices the data array given in argument 'data' in pieces
    *              of the length given in argument 'sliceLengthInByte'.
    *              Each slice is an array of byte of the given length and 
    *              represents as an element of the return array.
    *              Thus the return array is a 2 dimensional byte array of 
    *              the dimension 'n * sliceLengthInByte' where n is 
    *              the number of slices.
    *
    * @param data: The data array to slice.
    * @param sliceLengthInByte: The length of the slices. That value should match
    *                           wiht the encryption block length.
    *
    ...
    */
    export function sliceData(data: Array<number>, sliceLengthInByte: number = 4): Array<Array<number>>
    {
      ...
      var _resultArray: Array<Array<number>> = new Array<Array<number>>();

      if (sliceLengthInByte <= 0 || sliceLengthInByte > 255)
      {
        throw new TS.ArgumentOutOfRangeException("sliceLengthInByte", sliceLengthInByte, "Argument 'sliceLengthInByte' must be a value in range [1..255].");
      }//END if

      if ((data.length % sliceLengthInByte) != 0)
      {
        throw new TS.InvalidOperationException("Slicing the data into blocks of length " + sliceLengthInByte.toString() + " failed because the data length is not a multitude of the required slice length.");
      }//END if
      ...
      {
        _resultArray.push(_data.slice(0, sliceLengthInByte));
        _data = _data.slice(sliceLengthInByte);
      }//END while
    ...
    /**
    * @description An implementation of the padding algorythm as described 
    *              in RFC2315. That algorythm is also known as PKCS7 padding.
    *
    *
    * @param data: The data array which gets unpadded.
    * @param padLengthInByte: The length of the padding. That value should match
    *                         wiht the encryption block length.
    * 
    ...
    * @throws {TS.ArgumentOutOfRangeException}
    */
    export function padData(data: Array<number>, padLengthInByte: number = 16): Array<number>
    {
      var _resultArray: Array<number> = new Array<number>();
      var _reminder: number;
      var _index: number;
      var _data: Array<number>;

      _data = data.slice();
      _reminder = data.length % padLengthInByte;

      if (padLengthInByte <= 0 || padLengthInByte > 255)
      {
        throw new TS.ArgumentOutOfRangeException("padLengthInByte", padLengthInByte, "Argument 'padLengthInByte' must be a value in range [1..255] in function TS.Security.AES.padData.");
      }//END if

      if (_reminder == 0)
      {
        for (_index = padLengthInByte; _index > 0; _index--)
        {
          _data.push(padLengthInByte);
        }//END if
      }
      else
      {
        for (_index = padLengthInByte - _reminder; _index > 0; _index--)
        {
          _data.push(padLengthInByte - _reminder);
        }//END if
      }
    ...
    //TODO: Crate the test functions. Add descripion
    /**
    * @description Removes the pad bytes from the data which were 
    *              added by the padData function.
    * 
    * @see-also: {TS.Security.padData} 
    * 
    * @param data: The data array which gets padded.
    * 
    ...
    */
    export function unpadData(data: Array<number>): Array<number>
    {
      ...
      _padLengthInByte = data[data.length - 1];

      if (((data.length - _padLengthInByte) < 0) || (_padLengthInByte > 255))
      {
        throw new TS.ArgumentException("data", data, "The 'data' given in function 'unpadData' appears to be not a padded byte array.");
      }//END if

      _resultArray = data.slice(0, data.length - _padLengthInByte);
